...
- (void)show:(CDVInvokedUrlCommand*)command withAnimation:(BOOL)animated
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.themeableBrowserViewController != nil) {
            [self.navigationController pushViewController:self.themeableBrowserViewController animated:YES];
//            [self.viewController presentViewController:nav animated:animated completion:nil];
        }
    });
}

- (void)getNavController {
...
- (void)close
{
    ...
    // Run later to avoid the "took a long time" log message.
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.navigationController popViewControllerAnimated:_browserOptions.disableAnimation];
